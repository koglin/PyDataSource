...
class Impbox(PyDataSource.Detector):
    ...
    def add_filter(self, filter=None, signal_width=10):
        """Add a filter
            filter: filter array -- if not provided use step filter of width signal_width
            signal_width: width of signal [default=10]
        """
        if not filter:
